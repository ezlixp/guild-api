...
public class ScrollableContainer extends ScrollableWidget {
    ...
    private final ArrayList<ClickableWidget> clickables = new ArrayList<>();
    ...
    public void addClickableChild(ClickableWidget clickableElement) {
        drawables.add(clickableElement);
        clickables.add(clickableElement);
        maxHeight = Math.max(maxHeight, clickableElement.getBottom() - this.getY());
    }
    ...
    private boolean widgetClicked(ClickableWidget target, double mouseX, double mouseY) {
        return target.active && target.visible && mouseX >= (double) target.getX() && mouseY + this.getScrollY() >= (double) target.getY() && mouseX < (double) (target.getX() + target.getWidth()) && mouseY + this.getScrollY() < (double) (target.getY() + target.getHeight());
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        while (!removeables.isEmpty()) {
            ...
            if (toRemove instanceof ClickableWidget) {
    ...
    protected void renderContents(DrawContext context, int mouseX, int mouseY, float delta) {
        for (Drawable drawable : drawables) {
            drawable.render(context, mouseX, mouseY + (int) this.getScrollY(), delta);
        }
    }
}
