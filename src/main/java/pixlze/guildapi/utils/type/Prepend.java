...
public enum Prepend {
    DEFAULT(Text.literal("[Guild API] ").setStyle(Style.EMPTY.withColor(Formatting.GOLD)), Text.literal("[Guild API] ").setStyle(Style.EMPTY.withColor(Formatting.GOLD))),
    EMPTY(Text.empty(), Text.empty()),
    ...
    public MutableText get() {
        if (lastBadge.equals(TextUtils.parseStyled(prepend, TextParseOptions.DEFAULT)) && linesSinceBadge < 18) return blockIndicator.copy();
        else {
    ...
    public MutableText getWithStyle(Style style) {
        ...
        if (lastBadge.equals(TextUtils.parseStyled(prependWithStyle, TextParseOptions.DEFAULT)) && linesSinceBadge < 18) return blockIndicator.copy().fillStyle(style);
        else {
